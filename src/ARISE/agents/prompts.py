...
def system_prompt(agent_id: int, num_agents: int, max_agents: int, task: str, tools: str = "None.", role: str | None = None) -> str:
    last_agent_id = num_agents - 1
    role_text = role if role else "Unassigned"
    prompt = f"""You are agent {agent_id} in a mesh of {num_agents} agents (IDs 0 through {last_agent_id}).

You have the role of {role_text}.
...
Your final output can be a tool call, if this is the case, call the tool and submit a summary of the tool call in write output.
...
- assign_role — Assign yourself a role. Set recipient_id to -1. Content will be the role title: a short description of the role.
...
Example actions for a weather agent communicating with itinerary agent with ID 1:
{{'actions': [
...
- encompasses more than one responsibility
