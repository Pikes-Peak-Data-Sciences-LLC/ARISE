...
class ARISEMesh:
    def __init__(self, input_text: str, num_agents: int, max_agents: int) -> None:
        ...
        self.agents = [GenericAgent(agent_id=i, llm=BedrockClient(alt_system_prompt(i, num_agents, max_agents, input_text))) for i in range(num_agents)]
    ...
    def spawn_agent(self, _role: str) -> None:
        ...
        llm = BedrockClient(
            alt_system_prompt(new_id, new_id + 1, self.max_agents, self.input_text)
        )
